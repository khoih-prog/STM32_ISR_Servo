...
STM32_ISR_Servo STM32_ISR_Servos;  // create servo object to control up to 16 servos
...
int STM32_ISR_Servo::setupServo(uint8_t pin, int min, int max)
{
  int servoIndex;
...
bool STM32_ISR_Servo::setPosition(unsigned servoIndex, int position)
{
...
int STM32_ISR_Servo::getPosition(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return -1;
...
bool STM32_ISR_Servo::setPulseWidth(unsigned servoIndex, unsigned int pulseWidth)
{
  if (servoIndex >= MAX_SERVOS)
    return false;
...
unsigned int STM32_ISR_Servo::getPulseWidth(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return 0;
...
void STM32_ISR_Servo::deleteServo(unsigned servoIndex)
{
...
bool STM32_ISR_Servo::isEnabled(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return false;
...
bool STM32_ISR_Servo::enable(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return false;
...
bool STM32_ISR_Servo::disable(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return false;
...
bool STM32_ISR_Servo::toggle(unsigned servoIndex)
{
  if (servoIndex >= MAX_SERVOS)
    return false;
...
int STM32_ISR_Servo::getNumServos()
{
  return numServos;
}
